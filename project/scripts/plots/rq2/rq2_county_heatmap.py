"""Q2: Heatmap of county mean CES score by year."""
...
def main() -> None:
    ...
    pivot = pivot[[2014, 2019, 2021]]

    data = pivot.values

    fig, ax = plt.subplots(figsize=(7, max(6, len(pivot) * 0.18)))
    im = ax.imshow(data, aspect="auto", cmap="viridis")
    ax.set_yticks(np.arange(len(pivot)))
    ax.set_yticklabels(pivot.index, fontsize=6)
    ax.set_xticks([0, 1, 2])
    ax.set_xticklabels([2014, 2019, 2021])
    ax.set_title("Heatmap: County Mean CES Scores by Year")
    plt.colorbar(im, ax=ax, label="Mean CES Score")
    fig.tight_layout()

    out = fig_dir / "rq2_county_heatmap.png"
    fig.savefig(out, dpi=150)
    print(f"Wrote {out}")

    data_out = tbl_dir / "rq2_county_heatmap.csv"
    export = pivot.copy()
